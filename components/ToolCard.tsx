    ...
    >
      ...
      <div className="flex flex-1 flex-col p-3 md:p-4">
        <div className="mb-1.5 flex items-start gap-2">
          <span className="flex h-7 w-7 flex-none items-center justify-center rounded-lg text-sm md:h-8 md:w-8 md:text-base" style={{ background: bg }} aria-hidden="true">{icon}</span>
          <h2 className="m-0 pt-0.5 text-sm leading-snug md:text-lg">{title}</h2>
        </div>
        <p className="mb-0 mt-auto text-[11.5px] leading-normal text-ink-secondary md:mt-1 md:flex-1 md:text-[13.5px] md:leading-relaxed">
          <span className="md:hidden">{short}</span>
          <span className="hidden md:inline">{desc}</span>
        </p>
        <div className="mt-3 hidden items-center justify-between gap-2 border-t border-border pt-3 md:flex">
          <span className="font-mono text-[11px] text-ink-faint">{href}</span>
          <span className="text-[13px] font-semibold text-primary">เปิดใช้งาน ›</span>
        </div>
      </div>
    </Link>
  );
