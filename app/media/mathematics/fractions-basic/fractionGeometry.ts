...
export function stripBounds(index: number, parts: number, unequal = false): { x: number; width: number } {
  ...
  // ปัดผลต่างอีกชั้นเพราะการลบเลขทศนิยมสองตัวคืนเศษ float กลับมา
  // (16.667 - 0.001 ได้ 16.665999999999997) ซึ่งจะไปโผล่ใน attribute ของ SVG ตรง ๆ
  // ปลอดภัยเพราะทั้งสองตัวตั้งเป็นทศนิยม 3 ตำแหน่งอยู่แล้ว ผลต่างจริงจึงเป็น 3 ตำแหน่งเสมอ
  const width = round(round(after * 100) - x);
  return { x, width };
}
