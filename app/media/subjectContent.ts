import type { SubjectPageContent } from "./_components/SubjectResourcePage";

export const SUBJECT_CONTENT: Record<"mathematics" | "science" | "thai", SubjectPageContent> = {
  mathematics: {
    slug: "mathematics", name: "คณิตศาสตร์", icon: "🔢", accent: "#C85C12", soft: "#FFF0E4",
    headline: "เกมและสื่อการสอนคณิตศาสตร์ สำหรับครูประถม",
    intro: "เกมคณิตศาสตร์สำหรับใช้ทั้งห้อง ฝึกคิดเลขเร็ว สมการ เศษส่วน ร้อยละ และการประมาณค่า ผ่านภารกิจตัดสายระเบิดที่ตื่นเต้น เล่นได้ทันทีบนจอหน้าชั้นเรียน",
    topics: ["คิดเลขเร็ว", "สมการ", "เศษส่วน", "ร้อยละ", "การประมาณค่า"],
    resources: [
      { title: "ถอดรหัสบอมบ์ตัวเลข", description: "แก้โจทย์ เลือกตัดสายคำตอบที่ถูกต้อง และหยุดระเบิดก่อนหมดเวลา", grades: "ป.3–ป.6 · 5–10 นาที", type: "คิดเลขเร็ว", href: "/media/mathematics/math-bomb-defusal" },
    ],
    outcomes: ["เชื่อมแนวคิดนามธรรมกับภาพและสถานการณ์ใกล้ตัว", "สื่อสารกระบวนการคิดด้วยคำพูด ภาพ และสัญลักษณ์", "ตรวจสอบความสมเหตุสมผลของคำตอบได้ด้วยตนเอง"],
  },
  science: {
    slug: "science", name: "วิทยาศาสตร์", icon: "🔬", accent: "#24805F", soft: "#EAF7E4",
    headline: "เกมและสื่อการสอนวิทยาศาสตร์ เรียนรู้ผ่านการสังเกต",
    intro: "เกมวิทยาศาสตร์สำหรับพาเด็กวิเคราะห์สถานการณ์ ตั้งคำถาม และใช้หลักฐานแก้ปัญหา ครอบคลุมสิ่งมีชีวิต สสาร แรง พลังงาน ไฟฟ้า โลก และอวกาศ",
    topics: ["สิ่งมีชีวิต", "สสาร", "แรงและพลังงาน", "โลกและอวกาศ", "กระบวนการสืบเสาะ"],
    resources: [
      { title: "ห้องทดลองฉุกเฉิน", description: "วิเคราะห์สถานการณ์ เลือกวิธีแก้ปัญหา และกู้ระบบห้องทดลองให้ครบ 10 ภารกิจ", grades: "ป.3–ป.6 · 8–12 นาที", type: "วิทยาศาสตร์รอบตัว", href: "/media/science/science-lab-crisis" },
    ],
    outcomes: ["ตั้งคำถามและคาดการณ์จากความรู้เดิม", "เก็บและใช้หลักฐานจากการสังเกตหรือทดลอง", "อธิบายปรากฏการณ์ด้วยเหตุผลและคำศัพท์วิทยาศาสตร์"],
  },
  thai: {
    slug: "thai", name: "ภาษาไทย", icon: "📖", accent: "#B4477C", soft: "#FDE8F3",
    headline: "เกมและสื่อการสอนภาษาไทย อ่านคล่อง เขียนสื่อความ",
    intro: "พื้นที่รวมสื่อการสอนภาษาไทยสำหรับพัฒนาการอ่าน การเขียน และหลักภาษา ตั้งแต่สระ วรรณยุกต์ ไปจนถึงการอ่านจับใจความ ออกแบบสำหรับใช้บนจอหน้าชั้นเรียนและโทรศัพท์มือถือ",
    topics: ["อ่านออกเสียง", "สระและวรรณยุกต์", "คำและประโยค", "อ่านจับใจความ", "การเขียน"],
    resources: [
      { title: "สื่อการสอนภาษาไทย", description: "สื่อฝึกอ่าน สระ วรรณยุกต์ การประสมคำ หลักภาษา การเขียน และการอ่านจับใจความ กำลังอยู่ระหว่างเตรียมเนื้อหา", grades: "ป.1–ป.6 · กำลังเตรียม", type: "ภาษาไทย" },
    ],
    outcomes: ["อ่านออกเสียงคำและข้อความได้ถูกต้องคล่องแคล่ว", "จับใจความและใช้หลักฐานจากเรื่องที่อ่าน", "เรียบเรียงความคิดเป็นประโยคและข้อความที่สื่อความชัดเจน"],
  },
};
