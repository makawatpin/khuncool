...
export default function MediaGrid() {
  ...
  return <div>
    ...
    <div className="grid grid-cols-2 gap-[9px] md:grid-cols-4 md:gap-3">{items.map((item) => <article key={item.title} className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface-card">
      ...
      <div className="flex flex-1 flex-col p-[10px_11px_11px] md:p-[11px_12px_12px]">
        <div className="mb-[5px] flex flex-wrap gap-[5px]"><span className="rounded-[6px] bg-[#EEEEFD] px-[6px] py-[2px] text-[10px] font-semibold text-primary">{item.topic}</span><span className="rounded-[6px] bg-[#FFF3CC] px-[6px] py-[2px] text-[10px] font-semibold text-[#8A6A00]">{item.status}</span></div>
