...
import { CASES, EEAT_CARDS, FAQS, MEDIA, RELATED_SUBJECTS, RELATED_TOOLS, TRUST_CHIPS } from "./data";
...
export default function ComputerMediaPage() {
  return <main className="flex-1 w-full max-w-[1160px] mx-auto bg-white">
    ...
    <section className="px-4 pb-[22px] pt-3 md:grid md:grid-cols-[1.15fr_0.85fr] md:items-start md:gap-11 md:px-8 md:pb-[30px] md:pt-4">
      <div><h1 className="m-0 mb-2.5 text-[26px] leading-[1.32] md:mb-3.5 md:max-w-[18ch] md:text-[40px] md:leading-[1.24]">สื่อวิทยาการคำนวณ สนุกได้แม้ห้องไม่มีคอม</h1><p className="m-0 mb-3.5 text-sm leading-[1.7] text-ink-secondary md:mb-4 md:max-w-[60ch] md:text-[15.5px] md:leading-[1.75]">เกมและสื่อคอมพิวเตอร์สำหรับครูประถม แยกตามทักษะ Coding การพิมพ์ ความปลอดภัยดิจิทัล และฮาร์ดแวร์ ฉายขึ้นจอให้นักเรียนช่วยกันคิดเป็นทีม หรือเปิดเล่นรายคนผ่านเบราว์เซอร์ได้</p><div className="flex flex-wrap gap-[7px] md:gap-2">{TRUST_CHIPS.map((chip) => <span key={chip} className="rounded-pill bg-success-bg px-2.5 py-[5px] text-[11.5px] font-semibold text-success md:px-3 md:py-1.5 md:text-[12.5px]">{chip}</span>)}</div></div>
      <aside className="mt-6 hidden rounded-card-lg border border-border bg-surface-light p-[20px_22px] md:block"><div className="mb-2 font-mono text-[10.5px] font-semibold tracking-[.08em] text-primary">เริ่มเกมนี้ก่อน</div><h2 className="m-0 mb-2 text-lg">รู้จักโลกคอมพิวเตอร์ก่อนเริ่ม Coding</h2><p className="m-0 mb-4 text-[13px] leading-[1.65] text-ink-secondary">เด็กเริ่มจากแยกสิ่งที่จับต้องได้กับโปรแกรมที่สั่งงาน แล้วค่อยต่อยอดสู่การเรียงคำสั่งในเกมเขาวงกตโค้ดดิ้ง</p><div className="flex items-center gap-3 rounded-2xl border border-border bg-white p-3.5"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#FFF0E4] text-2xl">🖥️</span><div><div className="font-anuphan text-[15px] font-semibold">ภารกิจคัดแยกโลกดิจิทัล</div><div className="text-xs text-ink-secondary">ป.1–ป.4 · 8–12 นาที</div></div></div></aside>
    </section>

    <section className="px-4 pb-6 md:px-8 md:pb-[34px]" aria-labelledby="media-title"><h2 id="media-title" className="m-0 mb-1.5 text-lg md:text-2xl">เลือกตามทักษะที่กำลังสอน</h2><p className="m-0 mb-3 max-w-[64ch] text-[13px] leading-[1.65] text-ink-secondary md:mb-[18px] md:text-[14.5px]">การ์ดแต่ละเกมบอกระดับชั้น เวลา และผลลัพธ์การเรียนรู้ เพื่อให้ครูหยิบไปวางในแผนการสอนได้ง่าย</p><MediaGrid /></section>
