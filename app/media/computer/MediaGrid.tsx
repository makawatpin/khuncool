...
export default function MediaGrid() {
  ...
  return <div>
    ...
    {items.length > 0 ? <div className="grid grid-cols-2 gap-[9px] md:grid-cols-4 md:gap-3">
      ...
      </>; return item.href ? <Link href={item.href} key={item.title} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface-card text-inherit no-underline hover:border-[#C6C9FB]">{content}</Link> : <article key={item.title} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-surface-card">{content}</article>; })}
