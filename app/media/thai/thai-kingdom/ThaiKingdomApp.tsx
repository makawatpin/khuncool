...
export default function ThaiKingdomApp() {
  ...
  return <div {...stageProps} className="kc-stage"><div className={`kc-stage-body ${styles.body}`} onMouseOver={hoverSfxDelegate}>
    ...
    {screen === "home" && <main className={`${styles.screen} ${styles.home}`} data-stage="home"><div className={styles.heroCopy}><span className={styles.eyebrow}>ภาษาไทย ป.1 · อ่านออก เขียนได้</span><h1>อาณาจักรภาษาไทย</h1><h2>อ่านออก เขียนได้</h2><p>รู้จักพยัญชนะ ตามหาสระ แล้วสร้างคำให้รถไฟเดินทาง!</p>{latest && <span className={styles.latest}>{latest}</span>}</div><div className={styles.homeRight}><img className={styles.heroArt} src="/assets/game-covers/thai-kingdom.webp" width={1200} height={675} fetchPriority="high" decoding="async" alt="รถไฟ หนังสือ และโรงงานสร้างคำในอาณาจักรภาษาไทย" /><div className={styles.modeGrid}>{MODES.map((card) => <button key={card.mode} type="button" className={`kc-tap ${styles.modeCard} ${styles[card.tone]}`} onClick={() => selectMode(card.mode)}><span>{card.icon}</span><span><strong>{card.title}</strong><small>{card.description}</small></span><b>→</b></button>)}</div></div></main>}
