...
export type SeoClusterArticleConfig = {
  slug: string;
  title: string;
  description: string;
  cover: string;
...
export function SeoClusterArticle({ config }: { config: SeoClusterArticleConfig }) {
  ...
  return (
    <main className="flex-1 w-full max-w-[1160px] mx-auto bg-white">
      ...
      <article className="mx-auto max-w-[800px] px-4 pb-12 pt-6 md:px-8 md:pt-9">
        ...
        <h2 className="mt-9 text-xl md:text-2xl">{config.listHeading}</h2>
        <p className="text-[15px] leading-[1.85] text-[#2E3440]">{config.listIntro}</p>
        <div className="mt-4 grid gap-3">
          {config.items.map((item, index) => (
            <section key={item.title} className="rounded-2xl border border-border bg-surface-light p-4 md:p-5">
              <h3 className="m-0 text-[17px] leading-[1.5] md:text-lg"><span className="mr-2 text-primary">{index + 1}.</span>{item.title}</h3>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {config.guide.map((item) => (
            <section key={item.title} className="rounded-2xl border border-[#CDEEE3] p-4">
              ...
              {item.href && <Link href={item.href} className="mt-3 inline-block text-sm font-bold text-[#0A7A66]">{item.linkLabel ?? "อ่านเพิ่มเติม"} ›</Link>}
            </section>
          ))}
        </div>
